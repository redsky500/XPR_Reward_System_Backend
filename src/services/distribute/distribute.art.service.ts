...
const minHoldingAmountForArt = parseInt(process.env.HOLDING_AMOUNT_FOR_FAUCET);

const getAccountRegisteredOpulArt = async () => {
  const pageSize = 100; // Number of documents to retrieve per query
  let page = 0;
  const accounts: any[] = [];
  while (true) {
    const _stakers = await OpulenceArt.find()
      .skip(page * pageSize)
      .limit(pageSize);
...
async function calcRewardAndDrop() {
  ...
  const dailyReward = (parseInt(process.env.REWARD_FOR_ART_TIER_0) + parseInt(process.env.REWARD_FOR_ART_TIER_1) + parseInt(process.env.REWARD_FOR_ART_TIER_2)) / 365;
  ...
  const stakers = await getAccountRegisteredOpulArt();
  if (stakers.length === 0) {
  ...
  const counts = await getCountsForArt();
  let tier_count_0 = 0;
  let tier_count_1 = 0;
  let tier_count_2 = 0;
  ...
  const tier_daily_rewards = [
    Math.floor(parseInt(process.env.REWARD_FOR_ART_TIER_0) * Math.min(1 / tier_count_0, OPUL_REWARD_MAX_PERCENT / 100) / 365 * 1e6),
    Math.floor(parseInt(process.env.REWARD_FOR_ART_TIER_1) * Math.min(1 / tier_count_1, OPUL_REWARD_MAX_PERCENT / 100) / 365 * 1e6),
    Math.floor(parseInt(process.env.REWARD_FOR_ART_TIER_2) * Math.min(1 / tier_count_2, OPUL_REWARD_MAX_PERCENT / 100) / 365 * 1e6),
  ];
  if(Object.keys(counts).length > stakers.length) {
    const dropPromises = stakers.map(async (staker, index) => {
      ...
      if(parseFloat(balance || "0") < minHoldingAmountForArt) return;
